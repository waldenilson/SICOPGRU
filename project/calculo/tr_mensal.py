...
response_consulta  = "/calculo/tr_mensal/consulta/"
...
def consulta(request):
    if request.method == "POST" and len(request.POST['ano'])>0:
        ...
        lista = TbtrMensal.objects.filter( ano=ano)#, tbdivisao__id = AuthUser.objects.get( pk = request.user.id ).tbdivisao.id )
    else:
        lista = TbtrMensal.objects.all()#.filter( tbdivisao__id = AuthUser.objects.get( pk = request.user.id ).tbdivisao.id )
    lista = lista.order_by( 'ano','mes' )
